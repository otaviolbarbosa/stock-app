...
const Stock = () => {
  const { stockSymbol, quote, quoteError } = useSelector(
    ({ stockReducer }: RootState) => stockReducer
  );
  ...
  const isFavorite = () =>
    favorites.length > 0 && favorites.some((fav) => fav.symbol === quote.symbol);
    ...
    !quoteError && (
      <SC.Container>
        ...
        <SC.Info>
          ...
          <SC.Column>
            <SC.StockPrice>
              {drawArrow()}${roundedNumber(quote.latestPrice)}
            </SC.StockPrice>
            <SC.Row>
              <SC.StockChange isPositive={handleIsPositive()}>
